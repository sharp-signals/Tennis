...
PANEL_GUIDANCE = {
    ...
    "SNAPSHOT_RECONCILIATION_V1": "Reconcilia jogos, versões, identidades elegíveis e snapshots numa janela factual. Colisão significa que o provider reutilizou a mesma key para jogadores diferentes; não é ausência silenciosa nem autoriza backfill.",
    "GUERRA_SELECTION_V1": "Histórico preservado da seleção manual GUERRA. Esta metodologia está SUPERSEDED e não alimenta a monetização GREEN.",
...
METRIC_HELP = {
    ...
    "Resultado GREEN": "Lucro ou prejuízo realizado da simulação de €10 por leg GREEN válida e liquidada.",
    "Stake resolvida GREEN": "€10 por cada leg GREEN válida concluída, incluindo voids.",
    "Exposição pendente GREEN": "€10 por cada leg GREEN válida ainda sem settlement.",
    "ROI GREEN": "Resultado GREEN realizado dividido pela stake resolvida; não inclui exposição pendente.",
    "Legs resolvidas": "Legs GREEN válidas com resultado WIN, LOSS, VOID ou PUSH verificável.",
    "Legs pendentes": "Legs GREEN válidas ainda sem resultado final; contam apenas como exposição aberta.",
    "Legs excluídas": "Registos rejeitados por manifest de integridade, duplicação ou falta de prova factual.",
    "Vitórias GREEN": "Legs GREEN liquidadas como vitória.",
    "Derrotas GREEN": "Legs GREEN liquidadas como derrota.",
    "Voids GREEN": "Legs anuladas/PUSH: concluídas, com P/L simulado igual a zero.",
    "Matchups observados": "Jogos distintos apresentados na janela, depois de agrupar versões do mesmo encontro.",
    "Identidades elegíveis": "Identidades distintas presentes na telemetria operacional da janela.",
    "Ligação exata": "Identidades cuja key e jogadores/contexto coincidem bilateralmente com o snapshot persistido.",
    "Colisões de ID": "Keys do provider já ocupadas por jogadores diferentes; validation e PAPER são bloqueados.",
    "Sem snapshot elegível": "Identidades elegíveis sem ligação segura por colisão, key ausente ou identidade insuficiente.",
    "Snapshots criados": "Fotografias realmente first-write persistidas dentro da janela; nunca reconstruídas ex post.",
    "Snapshots liquidados": "Snapshots criados na janela que já têm vencedor bilateralmente validado.",
    "Cobertura elegível → snapshot": "Percentagem de identidades elegíveis ligadas com segurança ao snapshot persistido.",
...
.green-money-hero{position:relative;margin:0 0 16px;padding:clamp(18px,2.5vw,28px);border:1px solid #298765;border-radius:16px;background:radial-gradient(circle at 92% 8%,rgba(53,212,154,.16),transparent 38%),linear-gradient(145deg,#10261f,#111923 64%);overflow:hidden}.green-money-hero.DEGRADED{border-color:#9a762b}.green-money-hero.UNAVAILABLE{border-color:var(--line)}.green-money-head{display:flex;justify-content:space-between;gap:18px;align-items:flex-start}.green-money-kicker{color:var(--green);font-size:11px;font-weight:750;letter-spacing:.11em;text-transform:uppercase}.green-money-hero.DEGRADED .green-money-kicker{color:var(--yellow)}.green-money-hero h2{margin:5px 0 4px;font-size:clamp(18px,2.2vw,27px)}.green-money-status{border:1px solid currentColor;border-radius:999px;padding:4px 9px;font-size:10px;font-weight:750;letter-spacing:.08em}.green-money-total{position:relative;display:inline-block;padding-right:34px}.green-money-total>.help-trigger{top:22px;right:0}.green-money-result{font-size:clamp(38px,6vw,68px);line-height:1;font-weight:820;margin:20px 0 16px;letter-spacing:-.035em}.green-money-result.positive{color:var(--green)}.green-money-result.negative{color:var(--red)}.green-money-result.unavailable{color:var(--dim)}.green-money-hero .metrics{grid-template-columns:repeat(4,minmax(0,1fr))}.green-money-hero .metric{background:rgba(17,34,49,.82)}.green-money-warning{margin:14px 0 0;color:#c3d1df;font-size:12px}.superseded{opacity:.78;border-style:dashed}.superseded .eyebrow{color:var(--yellow)}
.reconciliation-panel{border-color:#755f2a}.reconciliation-panel .health{margin-bottom:12px}.reconciliation-panel .metrics{grid-template-columns:repeat(4,minmax(0,1fr))}
@media(max-width:900px){.shell{grid-template-columns:1fr}.status-strip{grid-template-columns:1fr}.green-money-hero .metrics,.reconciliation-panel .metrics{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:560px){.dashboard-legend{display:grid;grid-template-columns:1fr}.legend-item{border-radius:10px}.flat-sim{padding:12px}.help-copy{font-size:12px}.panel-guide{max-width:none}.green-money-hero{padding:16px}.green-money-head{display:block}.green-money-status{display:inline-flex;margin-top:9px}.green-money-result{font-size:42px}.green-money-hero .metrics,.reconciliation-panel .metrics{grid-template-columns:1fr 1fr}}
"""
...
function greenMonetizationHero(){const g=DATA.green_monetization_v1||{},status=g.status||'UNAVAILABLE',hasResult=Number(g.resolved_entries||0)>0&&typeof g.net_profit_eur==='number',result=hasResult?eur(g.net_profit_eur,true):'N/D',reasons=Object.entries(g.exclusion_reasons||{}).map(([reason,count])=>`${esc(reason)}: ${val(count)}`).join(' · '),markets=Object.entries(g.by_market||{}).map(([name,row])=>`<div class="row"><span>${esc(name)}</span><b>${val(row.eligible_entries)} legs · ${eur(row.net_profit_eur,true)}</b></div>`).join('');return `<section class="green-money-hero ${esc(status)}" aria-labelledby="green-money-title"><div class="green-money-head"><div><div class="green-money-kicker">GREEN_MONETIZATION_V1 · €10 flat-stake</div><h2 id="green-money-title">Resultado operacional estimado — €10 por aposta GREEN</h2>${panelGuide('GREEN_MONETIZATION_V1')}</div><span class="green-money-status">${esc(status)}</span></div><div class="green-money-total"><div class="green-money-result ${hasResult&&g.net_profit_eur>0?'positive':hasResult&&g.net_profit_eur<0?'negative':'unavailable'}">${result}</div>${helpUI('Resultado GREEN')}</div><div class="metrics">${metric('Stake resolvida GREEN',g.resolved_stake_eur==null?null:eur(g.resolved_stake_eur))}${metric('Exposição pendente GREEN',g.pending_exposure_eur==null?null:eur(g.pending_exposure_eur))}${metric('ROI GREEN',g.roi_pct,'%')}${metric('Legs resolvidas',g.resolved_entries)}${metric('Legs pendentes',g.pending_entries)}${metric('Legs excluídas',g.excluded_entries)}${metric('Vitórias GREEN',g.wins)}${metric('Derrotas GREEN',g.losses)}${metric('Voids GREEN',g.void_entries)}</div><details class="audit-detail"><summary>Mercados, deduplicação e exclusões</summary><div class="rows">${markets||'<div class="empty">N/D — nenhuma leg auditável.</div>'}</div><p class="audit-caption">Deduplicação: ${esc(g.deduplication?.contract||'N/D')} · identidades únicas: ${val(g.deduplication?.unique_leg_identities)}</p>${g.excluded_entries?`<p class="audit-caption">Exclusões agregadas: ${reasons||'N/D'}</p>`:''}</details><p class="green-money-warning"><b>Simulação histórica/experimental.</b> Não é dinheiro real e não prova edge futuro. PAPER, SHADOW e REAL permanecem separados.</p></section>`}
function snapshotReconciliationPanel(){const r=DATA.snapshot_reconciliation_v1||{},c=r.counts||{},w=r.window||{},reasons=Object.entries(r.absence_reason_codes||{}).map(([reason,count])=>`<div class="row"><span>${esc(reason)}</span><b>${val(count)}</b></div>`).join('');return panel('Cobertura jogos → snapshots',`${esc(r.metric_id||'SNAPSHOT_COVERAGE_RECONCILIATION_V1')} · ${esc(w.start_date||'N/D')} a ${esc(w.end_date||'N/D')}`,`<span class="health ${esc(r.status||'UNAVAILABLE')}">${esc(r.status||'UNAVAILABLE')}</span>${panelGuide('SNAPSHOT_RECONCILIATION_V1')}<div class="metrics">${metric('Matchups observados',c.matchups_observed)}${metric('Versões de relatório',c.report_versions)}${metric('Identidades elegíveis',c.eligible_run_identities)}${metric('Ligação exata',c.snapshot_identity_matches)}${metric('Colisões de ID',c.provider_match_id_collisions)}${metric('Sem snapshot elegível',c.eligible_without_snapshot)}${metric('Snapshots criados',c.snapshots_created)}${metric('Snapshots liquidados',c.snapshots_settled)}${metric('Cobertura elegível → snapshot',r.conversion?.eligible_to_snapshot_pct,'%')}</div><details class="audit-detail"><summary>Ver reconciliação e reason codes</summary><div class="rows">${reasons||'<div class="empty">N/D</div>'}</div><p class="audit-caption">Linked exact: ${val(c.linked_exact_versions)} versões · self-described: ${val(c.self_described_versions)} versões · versões duplicadas/reruns: ${val(c.duplicate_report_versions)}. Sem backfill: ${r.integrity?.no_backfill===true?'sim':'N/D'}.</p></details>`,'wide reconciliation-panel','SNAPSHOT_RECONCILIATION_V1')}
