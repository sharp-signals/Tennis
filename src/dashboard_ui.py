...
function globalView(){
 ...
 if(cardsHost){cardsHost.insertAdjacentHTML('beforeend',`<div class="card"><div class="label">Versões com cor N/D</div><div class="value">${val(DATA.global.report_colors.UNAVAILABLE)}</div></div>`);cardsHost.insertAdjacentHTML('afterend','<p class="audit-caption">Jogos distintos agrupam versões do mesmo matchup. total_reports preserva a contagem legacy de ficheiros/versões. As cores e os filtros contam versões, não jogos; snapshots contam fotografias. Legs PAPER não são jogos independentes. Cor operacional não é resultado de aposta.</p>')}
